...
class UsageTrackerAsync:
    ...
    async def _init_database(self) -> None:
        """初始化SQLite数据库"""
        try:
            self.db = await aiosqlite.connect(str(self.db_file))
            await self.db.execute("PRAGMA journal_mode=WAL")  # 启用WAL模式提升并发性能
            await self.db.execute("PRAGMA synchronous=NORMAL")  # 平衡性能和安全性
            ...
            # 创建索引
            await self.db.execute("CREATE INDEX IF NOT EXISTS idx_date ON usage_records(date)")
            await self.db.execute("CREATE INDEX IF NOT EXISTS idx_user_date ON usage_records(user_id, date)")
            await self.db.execute("CREATE INDEX IF NOT EXISTS idx_model ON usage_records(model)")
    ...
    async def _flush_batch(self, batch: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            await self.db.executemany(insert_sql, records)
            await self.db.commit()
